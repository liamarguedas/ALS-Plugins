...
new bool:molotovDisabled[MAXPLAYERS+1];
new Handle:cvar_ignite = INVALID_HANDLE;
new Handle:cvar_ignite_time = INVALID_HANDLE;
new Handle:cvar_damage = INVALID_HANDLE;
new Handle:cvar_ff = INVALID_HANDLE;
...
public OnPluginStart()
{
  ...
  RegConsoleCmd("say_molotov", Cmd_Molotov); // For chat command !nomolotov
  RegConsoleCmd("say_team_molotov", Cmd_Molotov); // For team chat command !nomolotov
...
public ProjectileSpawned(entity)
{
	...
	if(molotovDisabled[client] || 0 >= molotovnumero[client])
		return;
...
public Action:Cmd_Molotov(client, args)
{
    if (args != 1)
    {
        PrintToChat(client, "\x04[SM_CSSMolotov]\x05 Usage: !molotov on/off");
        return Plugin_Handled;
    }
    ...
    if (StrEqual(arg, "on", false))
    {
        molotovDisabled[client] = false;
        PrintToChat(client, "\x04[SM_CSSMolotov]\x05 Molotovs have been enabled for you.");
    }
    else if (StrEqual(arg, "off", false))
    {
        molotovDisabled[client] = true;
        PrintToChat(client, "\x04[SM_CSSMolotov]\x05 Molotovs have been disabled for you.");
    }
    else
    {
        PrintToChat(client, "\x04[SM_CSSMolotov]\x05 Invalid argument. Use: !molotov on/off");
    }
    return Plugin_Handled;
}
